...
$projects_GroupId = filter_input(INPUT_POST, 'projects_GroupId',FILTER_SANITIZE_STRING);
...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if(empty($errors))
	{
		$sql = $db->prepare('UPDATE projects SET project_Name = :project_Name, project_Text = :project_Text, project_Date = :project_Date, projects_GroupId = :projects_GroupId, projects_Picture1 = :projects_Picture1, projects_Picture2 = :projects_Picture2, projects_Picture1HD = :projects_Picture1HD, projects_Picture2HD = :projects_Picture2HD WHERE project_Id = :project_Id');
		
		$sql->bindValue(':project_Id', $project_Id, PDO::PARAM_STR);
		...
		$sql->bindValue(':projects_GroupId', $projects_GroupId, PDO::PARAM_STR);
		...
		$sql->execute();
		header('location: editGroup.php?id='.$projects_GroupId);
		exit;
	}
...
else
{
	...
	$sql = $db->prepare('SELECT projects_Picture1, projects_Picture2, projects_Picture1HD, projects_Picture2HD, project_Id, project_Name, project_Text, project_Date, projects_GroupId FROM projects WHERE project_Id = :project_Id');
	$sql->bindValue(':project_Id', $project_Id, PDO::PARAM_INT);
	$sql->execute();
	$results = $sql->fetch(PDO::FETCH_OBJ);

	$project_Name = $results->project_Name;
	$project_Text = $results->project_Text;
	$project_Date = $results->project_Date;
	$projects_GroupId = $results->projects_GroupId;
	
	$projects_Picture1 = $results->projects_Picture1;
	$projects_Picture2 = $results->projects_Picture2;
	$projects_Picture1HD = $results->projects_Picture1HD;
	$projects_Picture2HD = $results->projects_Picture2HD;
...
<body>
	<div id="content">
    <form action="editProject.php?id=<?php echo $project_Id; ?>" method="post">

	    <h1>Edit Project - <?php  echo $project_Id; ?></h1>
	    <p>Make sure userId matches the right user, and group id matches the right project group.</p>
	    
        <div>
        	<label for="user_Id">Group ID</label>
        	<br/>
            <?php if(isset($errors['projects_GroupId'])): ?>
            <label for "projects_GroupId"><p class="error">Error! Enter their group id.</p></label>
            <?php endif; ?>
            <input id="projects_GroupId" name="projects_GroupId" value="<?php echo $projects_GroupId; ?>">
        </div>
        	<br/> 	    
         <div>
        ...
        <div class="clear-fix">
            ...
            <?php endif; ?>
            <input id="project_Date" name="project_Date" value="<?php echo $project_Date; ?>">
        </div>   
        	<br/>      	
         <div class="clear-fix">
        	...
        	<br/> 
        <div>
            <a href="editGroup.php?id=<?php echo $projects_GroupId ?>">&lt;&lt;Back</a>
